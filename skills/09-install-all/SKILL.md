...
description: 將 OpenCode 懶人包 00-09 以 opencode-* 名稱全部安裝至 ~/.config/opencode/skills，不在 ~/.agents 留副本，再逐項執行 00-08。
---
...
1. **長駐安裝**：將 repo 中 00～09 對應的 10 個 Skills 以 `opencode-*` 名稱安裝到 OpenCode 全域 Skills 目錄。
...
不要改成無參數的 `npx skills add`，也不要把 `.agents` 當成完成位置。最終以下 10 個 Skills 都必須實際存在於 `~/.config/opencode/skills/`，且資料夾名稱與 frontmatter `name` 相同：
...
確認 10 個 `opencode-*` Skills 已全部長駐後，依序載入並執行：
...
8. **opencode-workflow-skills** — 安裝 startup/shutdown/project-init
9. **opencode-draw** — 安裝 OpenCode 生圖 Skill

先列出 00–08，逐項詢問使用者是否執行。涉及登入、建立遠端 repo、修改既有專案或寫入外部目錄時仍須另外取得確認。已安裝且驗證正常的外部工具可以跳過執行，但不得因此刪除或漏裝對應的 `opencode-*` Skill。

執行 `opencode-workflow-skills` 後，另外確認 `startup`、`shutdown`、`project-init` 三個長駐 Skills 存在。`opencode-draw` 本身就是長駐生圖 Skill，不得只留下舊版 `draw` 或 `08-draw`。

## 完成標準

- 10 個 `opencode-*` Skills：全部實際存在於 `~/.config/opencode/skills/`，且 frontmatter 名稱與目錄相同。
...
- 最終預期為 13 個相關 Skills；只要缺少 `opencode-*` Skill、仍留舊編號副本或多出 `nlm-skill`，就不能標示為完整安裝。
